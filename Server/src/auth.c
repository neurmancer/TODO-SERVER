...
static int same_origin(const char *raw)
{
    ...
    if (o < 0 || h != 1 || s < 0 || !strcmp(site, "cross-site")) {
        return(0);
    }
...
{
    char cookies[4096];
    if (header(raw, "Cookie", cookies, sizeof(cookies)) != 1) {
        return(-1);
    }
    ...
    for (int i = 0; i < SESSION_COUNT; i++) {
        if (sessions[i].expires > time(NULL) && !CRYPTO_memcmp(token, sessions[i].token, 64)) {
            return(i);
        }
    }
    return(-1);
