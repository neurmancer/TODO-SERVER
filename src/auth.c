...
{
    char path[PATH_MAX];
    if (get_server_path(path, sizeof(path), "auth/credentials") != OK) return -1;
    struct stat info;
    if (lstat(path, &info) != 0) {
        if (errno == ENOENT) return 0;
        return -1;
    }
    if (!S_ISREG(info.st_mode) || (info.st_mode & 077) || info.st_uid != geteuid()) return -1;
    ...
    if (fields != 4 || strcmp(version, "pbkdf2-sha256-600000") ||
        !unhex(salt_hex, salt, 32) || !unhex(hash_hex, password_hash, 32)) return -1;
    enabled = 1;
    return 0;
}
        ...
        if ((size_t)(end - p) > n && !strncmp(p, name, n) && p[n] == '=') {
            ...
            for (const char *v = p + n + 1; v < end; v++) {
                ...
                if (c == '+') c = ' ';
                    ...
                    for (int j = 0; j < 2; j++) if (hex[j] >= 'A' && hex[j] <= 'F') hex[j] += 'a' - 'A';
...
int auth_handle(TLSClient *client, const char *method, const char *path, const char *raw, const char *body)
{
    ...
    if (get && !strcmp(path, "/login")) {
        ...
        FILE *file = NULL;
        if (get_server_path(filename, sizeof(filename), "frontend/login.html") == OK) file = fopen(filename, "r");
        if (!file) reply(client, 503, "text/plain", "", "Login page unavailable. Run build.sh update.");
        else {
    ...
    if (!get && !same_origin(raw)) {
        reply(client, 403, "application/json", "", "{\"error\":\"Request origin rejected.\"}"); return 1;
    }
    if (get && (!strcmp(path, "/style.css") || !strcmp(path, "/login.js") || !strcmp(path, "/favicon.png"))) return 0;
    ...
    if (post && !strcmp(path, "/login")) {
        ...
        for (size_t i = 0; i < sizeof(random); i++) sprintf(sessions[slot].token + i * 2, "%02x", random[i]);
